...
final_df_selected <- read_csv("C:\\Users\\jquintero24E\\Documents\\GitHub\\icu_mortality_model\\Analysis\\dataframe_mA.csv")
...
final_df_selected <- final_df_selected %>%
  mutate(
    ...
    epinephrine = ifelse(epinephrine == 1, 0, 1),
...
final_df_selected$epinephrine <- as.factor(final_df_selected$epinephrine)
...
# 3. SAVE DIVISION TO CSV
   ...
   write.csv(data_test, "data_test_A.csv")
...
fit_cforest <- train(outcome_and_exposure,                              # 18 h  
                data = data_train_without_ID %>% as.data.frame(),    
                method = "Rborist",
                trControl = fitControl,
                verbose = F,
...
```
The model xgb has the best balanced accuracy, but there are other effective models.All models have a high performance F1, so to evaluate other models the confusion matrix and Specificity can be considered. Models rf and ada are considered to be effective models.
...
fit_best_3 <- readRDS("ada_model.Rda")
...
ggplot(precision_recall_df3, aes(x = recall, y = precision)) +
  ...
  geom_point() +
  labs(x = "Recall", y = "Precision") +
  ggtitle("Precision vs. Recall Curve  MODEL 3 (ada)")
...
ggplot(importance3, aes(x = Overall, y = reorder(rownames(importance), Overall))) +
  geom_bar(stat = "identity", width = 0.5, fill = "blue") +
  labs(title = "IMPORTANCE OF THE MODEL 3 (ada) VARIABLES",
       x = "Variables",
       y = "Relative importance (%)") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 7)) +
