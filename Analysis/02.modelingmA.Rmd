...
```
These graphs indicate that the best threshold is 0.3, but we will maintain 0.4 for uniformity with Models A, B and C.
...
data_test %>%
  mutate(pred = data_plot_rf$y,
         pred.lower = data_plot_rf$ymin,
         pred.upper = data_plot_rf$ymax) %>%
    dplyr::group_by(Cardiac_arrest) %>%
    dplyr::summarise(pred.mean = mean(pred),
            pred.low = mean(pred.lower),
            pred.up = mean(pred.upper)) %>%
  ungroup() %>%
  ggplot(aes(x = factor(Cardiac_arrest), y = pred.mean)) +
  geom_jitter(aes(y = pred, col = factor(Cardiac_arrest)),
              alpha = 0.07,
              data = data_test %>%
  mutate(pred = data_plot_rf$y,
         pred.lower = data_plot_rf$ymin,
