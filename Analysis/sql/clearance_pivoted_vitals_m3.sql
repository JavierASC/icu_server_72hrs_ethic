WITH ChartoffsetStatsHeartrate AS (
  ...
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND heartrate IS NOT NULL
...
ChartoffsetStatsRespiratoryrate AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND respiratoryrate IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsSpO2 AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND spo2 IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsNIBPSystolic AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND nibp_systolic IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsNIBPDiastolic AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND nibp_diastolic IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsNIBPMean AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND nibp_mean IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsTemperature AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND temperature IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsIBPSystolic AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND ibp_systolic IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsIBPDiastolic AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND ibp_diastolic IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsIBPMean AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_vital`
  WHERE chartoffset >= -1440 AND chartoffset <= 2880 AND ibp_mean IS NOT NULL
  GROUP BY patientunitstayid
),
patientdistinct AS (
