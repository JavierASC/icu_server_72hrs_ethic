WITH ChartoffsetStatsAlbumin AS (
  ...
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND albumin IS NOT NULL
...
ChartoffsetStatsBilirubin AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND bilirubin IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsBUN AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND BUN IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsCalcium AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND calcium IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsCreatinine AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND creatinine IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsGlucose AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND glucose IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsBicarbonate AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND bicarbonate IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsTotalCO2 AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND TotalCO2 IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsHematocrit AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND hematocrit IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsHemoglobin AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND hemoglobin IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsINR AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND INR IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsLactate AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND lactate IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsPlatelets AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND platelets IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsPotassium AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND potassium IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsPtt AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND ptt IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsSodium AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND sodium IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsWbc AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND wbc IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsBands AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND bands IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsAlt AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND alt IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsAst AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND ast IS NOT NULL
  GROUP BY patientunitstayid
),
ChartoffsetStatsAlp AS (
  ...
  FROM `physionet-data.eicu_crd_derived.pivoted_lab`
  WHERE chartoffset >= -1440 AND chartoffset < 0 AND alp IS NOT NULL
  GROUP BY patientunitstayid
),
patientdistinct AS (
