...
final_df_selected <- read_csv("C:\\Users\\jquintero24E\\Documents\\GitHub\\icu_mortality_model\\Analysis\\dataframe_m1.csv")
...
fit_cforest <- train(outcome_and_exposure,                              # 2,5 h  
                data = data_train_without_ID %>% as.data.frame(),    
                method = "Rborist",
                trControl = fitControl,
                verbose = F,
...
```
The model ada has the best balanced accuracy, but there are other effective models.All models have a high performance F1, so to evaluate other models the confusion matrix and Specificity can be considered. Models gbm and xgb are considered to be effective models.
