...
# Save the model with the highest performance. We will focuss on "Balanced_Accuracy.Mean"
...
# Although the xgbtree model has a superior balanced accuracy, since we prioritize guessing or classifying the dead based on the confusion matrix, we selected the gbm model.
...
fit_cforest <- train(outcome_and_exposure,
                data = data_train_without_ID,    
                method = "gbm",
                trControl = fitControl,
                verbose = F,
...
fit_best <- readRDS("gbm_model.Rda")
...
final_predictions_best = getFinalPredictions(fit_best, data_test)
final_predictions <- final_predictions_best
...
final_predictions_0.5 = getFinalPredictions(fit_best, data_test, prob_threshold = 0.5)
...
gbm_F1 <-F1_Score(final_predictions$obs, final_predictions$pred)

# PRINT

str_glue("gbm model F1 is {round(cforest_F1, digits = 3)}")
...
getFinalPredictions = function(fit_best_male, data_test_male, prob_threshold=0.4){                 # Change the fit to the best_model. 
...
# icu_mortality_72hrs
final_predictions_malw$obs <- as.factor(final_predictions_male$obs)
