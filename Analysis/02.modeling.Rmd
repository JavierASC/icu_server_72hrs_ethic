...
# Predictions for hours spent from hospital admission to ICU wards (prev_dept_los_hrs)
data_test %>%
  mutate(pred = data_plot$y,
         pred.lower = data_plot$ymin,
         pred.upper = data_plot$ymax) %>%
  dplyr::group_by(prev_dept_los_hrs) %>%
  dplyr::summarise(pred.mean = mean(pred),
            pred.low = mean(pred.lower),
            pred.up = mean(pred.upper)) %>%
  ungroup() %>%
  dplyr::mutate(prev_rounded = round(prev_dept_los_hrs, 1)) %>%
  dplyr::filter(prev_dept_los_hrs <= 100) %>%
  dplyr::filter(prev_rounded %in% seq(0, 100, 1)) %>%
  dplyr::group_by(prev_rounded) %>%
  dplyr::summarise(pred.mean = mean(pred.mean),
...
data_test %>%
  dplyr::mutate(pred = data_plot$y,
         pred.lower = data_plot$ymin,
         pred.upper = data_plot$ymax) %>%
  dplyr::group_by(prev_dept_los_hrs, apache_other) %>%
  dplyr::summarise(pred.mean = mean(pred),
            pred.low = mean(pred.lower),
            pred.up = mean(pred.upper)) %>%
  ungroup() %>%
  ...
  dplyr::filter(prev_dept_los_hrs <= 100) %>%
  dplyr::filter(prev_rounded %in% seq(0, 100, 1)) %>%
  dplyr::group_by(prev_rounded, apache_other) %>%
  dplyr::summarise(pred.mean = mean(pred.mean),
            pred.low = mean(pred.low),
            pred.up = mean(pred.up)) %>%
  ungroup() %>%
  dplyr::mutate(apache_other = ifelse(apache_other == "1", "Apache_other = No", "Apache_other = Yes"),
...
data_test %>%
  dplyr::mutate(pred = data_plot$y,
         pred.lower = data_plot$ymin,
         pred.upper = data_plot$ymax) %>%
  dplyr::group_by(prev_dept_los_hrs, apache_other, hist_diabetes_bin) %>%
  dplyr::summarise(pred.mean = mean(pred),
            pred.low = mean(pred.lower),
            pred.up = mean(pred.upper)) %>%
  ungroup() %>%
  ...
  dplyr::filter(prev_dept_los_hrs <= 100) %>%
  dplyr::filter(prev_rounded %in% seq(0, 100, 1)) %>%
  dplyr::group_by(prev_rounded, apache_other, hist_diabetes_bin) %>%
  dplyr::summarise(pred.mean = mean(pred.mean),
            pred.low = mean(pred.low),
            pred.up = mean(pred.up)) %>%
  ungroup() %>%
