...
# saveRDS(fit_lr, file = "lr_model.Rda") # Runs later
...
best_performing_model<-get(
  rownames(summary_resamps)[which(summary_resamps$F1.Median == max(summary_resamps$F1.Median))])
...
# This code does not have the expected result. By evaluating the models individually it is discovered that the best model is lr.
...
# Each model was evaluated manually, obtaining that the lr model is the one with the highest F1.

# Force lr as best model:
...
gbm_success <- mean(final_predictions_gbm$obs == final_predictions_gbm$pred)*100
rf_success <- mean(final_predictions_rf$obs == final_predictions_rf$pred)*100
xgb_success <- mean(final_predictions_xgb$obs == final_predictions_xgb$pred)*100
lr_success <- mean(final_predictions_lr$obs == final_predictions_lr$pred)*100
lda_success <- mean(final_predictions_lda$obs == final_predictions_lda$pred)*100
kernelpls_success <- mean(final_predictions_kernelpls$obs == final_predictions_kernelpls$pred)*100
ranger_success <- mean(final_predictions_ranger$obs == final_predictions_ranger$pred)*100
Linda_success <- mean(final_predictions_Linda$obs == final_predictions_Linda$pred)*100
cforest_success <- mean(final_predictions_cforest$obs == final_predictions_cforest$pred)*100
...
# Success
success <- round(c(lr_success, cforest_success, lda_success, kernelpls_success, Linda_success, gbm_success, xgb_success, ranger_success, rf_success),2)

# Dataframe
comparative_table <- data.frame(Models = Models, F1 = F1, Success = paste0(success, "%"))
...
You can avoid this code if you have saved the model (run the next section of the code in this case).
...
# I add a code to save and load the model.

# SAVE THE MODEL: you can save the model so you don't have to run it again.
...
# LOAD THE SELECTED MODEL (only if you have run "TRAINING ONLY OF THE SELECTED MODEL")

You can load the trained model if you have already trained it in a previous session.This saves training time.
The training of the model is in the previous section.
...
confusionMatrix(final_predictions_male$pred, final_predictions_male$obs, mode = "everything")
...
confusionMatrix(final_predictions_female$pred, final_predictions_female$obs, mode = "everything")
...
confusionMatrix(final_predictions_adolescence$pred, final_predictions_adolescence$obs, mode = "everything")
...
confusionMatrix(final_predictions_youth$pred, final_predictions_youth$obs, mode = "everything")
...
confusionMatrix(final_predictions_adulthood$pred, final_predictions_adulthood$obs, mode = "everything")
...
confusionMatrix(final_predictions_old_age$pred, final_predictions_old_age$obs, mode = "everything")
...
confusionMatrix(final_predictions_teach_true$pred, final_predictions_teach_true$obs, mode = "everything")
...
confusionMatrix(final_predictions_teach_false$pred, final_predictions_teach_false$obs, mode = "everything")
...
confusionMatrix(final_predictions_african_american$pred, final_predictions_african_american$obs, mode = "everything")
...
confusionMatrix(final_predictions_asian$pred, final_predictions_asian$obs, mode = "everything")
...
confusionMatrix(final_predictions_caucasian$pred, final_predictions_caucasian$obs, mode = "everything")
...
confusionMatrix(final_predictions_hispanic$pred, final_predictions_hispanic$obs, mode = "everything")
...
confusionMatrix(final_predictions_native_american$pred, final_predictions_native_american$obs, mode = "everything")
...
confusionMatrix(final_predictions_ethnicity_other$pred, final_predictions_ethnicity_other$obs, mode = "everything")
...
## Comparative table Option B (MODIFICAR)
...
# Success
success <- round(c(success_male, success_female, success_adolescence, success_youth, success_adulthood, success_old_age, success_teach_true, success_teach_false, success_african_american, success_asian, success_caucasian, success_hispanic, success_native_american, success_ethnicity_other),3)

# Dataframe
comparative_option_B <- data.frame(Groups = Groups, F1 = F1, Success = paste0(success, "%"))
