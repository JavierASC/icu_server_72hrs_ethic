...
final_df_selected <- read_csv("C:\\Users\\jquintero24E\\Documents\\GitHub\\icu_mortality_model\\Analysis\\dataframe_mB.csv")
...
# 3. SAVE DIVISION TO CSV
   ...
   write.csv(data_test, "data_test_B.csv")
...
```
The model xgb has the best balanced accuracy, but there are other effective models.All models have a high performance F1, so to evaluate other models the confusion matrix and Specificity can be considered. Models rf, gbm and ada are considered to be effective models.
...
fit_best_1 <- readRDS("xgb_model.Rda")
...
fit_best_3 <- readRDS("gbm_model.Rda")
...
fit_best_4 <- readRDS("ada_model.Rda")

#---

# CREATE THE getFinalPredictions FUNCTION.

getFinalPredictions = function(fit_best_4, data_test, prob_threshold=0.4){     
                                                                                           
  prediction_probabilities_tmp = predict(fit_best_4, newdata=data_test, type = "prob")        
...
final_predictions_best_4 = getFinalPredictions(fit_best_4, data_test)
...
#                     CONFUSION MATRIX (selected model 4)

str_glue("-------------------------------- Model 4: ada")

# The dependent variable must be of type factor.

# icu_mortality_72hrs
final_predictions_best_4$obs <- as.factor(final_predictions_best_4$obs)

# mortality_prediction_B
final_predictions_best_4$pred <- as.factor(final_predictions_best_4$pred)

# confusion matrix

confusionMatrix(final_predictions_best_4$pred, final_predictions_best_3$obs, mode = "everything")
...
ggplot(precision_recall_df1, aes(x = recall, y = precision)) +
  ...
  ggtitle("Precision vs. Recall Curve  MODEL 1 (xgb)")
...
ggplot(precision_recall_df3, aes(x = recall, y = precision)) +
  ...
  geom_point() +
  labs(x = "Recall", y = "Precision") +
  ggtitle("Precision vs. Recall Curve  MODEL 3 (gbm)")

#-------------------------------------------------------------------------------

# (Selected model 4)

precision_recall_df4 <- calculate_precision_recall(final_predictions_best_4$pred, final_predictions_best_4$obs)

ggplot(precision_recall_df4, aes(x = recall, y = precision)) +
  geom_line() +
  geom_point() +
  labs(x = "Recall", y = "Precision") +
  ggtitle("Precision vs. Recall Curve  MODEL 4 (ada)")
...
importance4 <- varImp(fit_best_4)

# ggplot

ggplot(importance1, aes(x = Overall, y = reorder(rownames(importance), Overall))) +
  geom_bar(stat = "identity", width = 0.5, fill = "blue") +
  labs(title = "IMPORTANCE OF THE MODEL 1 (xgb) VARIABLES",
       x = "Variables",
...
ggplot(importance3, aes(x = Overall, y = reorder(rownames(importance), Overall))) +
  geom_bar(stat = "identity", width = 0.5, fill = "blue") +
  labs(title = "IMPORTANCE OF THE MODEL 3 (gbm) VARIABLES",
       x = "Variables",
       y = "Relative importance (%)") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 7)) +
...
ggplot(importance4, aes(x = Overall, y = reorder(rownames(importance), Overall))) +
  geom_bar(stat = "identity", width = 0.5, fill = "blue") +
  labs(title = "IMPORTANCE OF THE MODEL 4 (ada) VARIABLES",
       x = "Variables",
       y = "Relative importance (%)") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 7)) +
...
final_predictions <- final_predictions_best_1
